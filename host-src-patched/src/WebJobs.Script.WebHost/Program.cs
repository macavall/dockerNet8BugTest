        ...
        public static IWebHostBuilder CreateWebHostBuilder(string[] args = null)
        {
            ...
            return AspNetCore.WebHost.CreateDefaultBuilder(args)
                .ConfigureKestrel(o =>
                {
                    ...
                    // Disable the host Kestrel's slow-upload rate monitor. By default
                    // MinRequestBodyDataRate is 240 B/s with a 5s grace period, so clients on
                    // slow/constrained links whose upload rate drops below that for >5s get a
                    // BadHttpRequestException ("Reading the request body timed out ... See
                    // MinRequestBodyDataRate"). This limit is not configurable from the function
